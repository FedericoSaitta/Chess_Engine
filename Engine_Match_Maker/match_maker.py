...
'''Now it is bug free hou should run some games, before it wasnt making the opening moves, also make your computer not shut down'''
def start_match(game_number):
    ...
    while True:
        ...
        if turn < 18:
            ...
            if move is (not None): make_move(board, move, dict)
            turn += 1
...
def write_pgn(old_move_log, engine_1, engine_2, draws):
    move_notation = []
    ...
    if engine_1 == 1:
        sentence = f'Old engine won '
    elif engine_2 == 1:
        sentence = f'New engine won '
    else:
        sentence = f'It was a draw '

    results_file.write(sentence + results + '\n')
def main():
    ...
    for i in range(100):
        ...
        write_pgn(dictionary['move_log'], white_1, black_1, draws_1)
